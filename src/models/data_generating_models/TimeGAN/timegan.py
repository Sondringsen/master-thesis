...
def timegan (ori_data, parameters):
  ...
  D_loss_real = binary_cross_entropy(tf.ones_like(Y_real), Y_real)
  D_loss_fake = binary_cross_entropy(tf.zeros_like(Y_fake), Y_fake)
  D_loss_fake_e = binary_cross_entropy(tf.zeros_like(Y_fake_e), Y_fake_e)
  D_loss = D_loss_real + D_loss_fake + gamma * D_loss_fake_e
            
  # Generator loss
  # 1. Adversarial loss
  G_loss_U = binary_cross_entropy(tf.ones_like(Y_fake), Y_fake)
  G_loss_U_e = binary_cross_entropy(tf.ones_like(Y_fake_e), Y_fake_e)
    
  # 2. Supervised loss
  mse = tf.keras.losses.MeanSquaredError()
  # G_loss_S = mse.call(H[:,1:,:], H_hat_supervise[:,:-1,:])
  G_loss_S = mse.call(H[:,:,:], H_hat_supervise[:,:,:])
  ...
  with tf.compat.v1.Session() as sess:
    ...
    saver.save(sess, 'data/params/timegan_model.ckpt')    
